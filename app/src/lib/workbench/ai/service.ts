...
export function validateQuestions(raw:unknown,count:number,profile:Resume|undefined,seeds:Corpus["questions"],version:string|null):QuestionPack {
  ...
  for(const q of parsed.data.questions){
    ...
    if(q.profileEvidence.some(e=>!profileFields.some(field=>field.includes(normalize(e)))))throw new WorkbenchError("INVALID_EVIDENCE","Gemini returned an unsupported profile excerpt. Retry generation.",502);
