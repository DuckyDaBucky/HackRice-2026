...
type Status={gemini:{configured:boolean;model:string};backboard:{configured:boolean};database:string};
...
export default function AiLab({profile,target,onResume}:{profile?:Resume;target:Target;onResume:()=>void}){
  ...
  const [attachProfile,setAttachProfile]=useState(false),[attachTarget,setAttachTarget]=useState(false),[useMemory,setUseMemory]=useState(false);
  ...
  return <section className={styles.lab} aria-label="AI playground">
    <div className={styles.heading}><div><p className={styles.eyebrow}>LIVE AI / PRIVATE PRACTICE</p><h2>Put the conversation to work.</h2><p>Chat with Gemini, try an interview, and choose what to remember.</p></div><span className={styles.pill}>Database disconnected</span></div>
    <div className={styles.layout}>
      <aside className={styles.context}>
        ...
        <hr/><h3>Provider connections</h3>{(["gemini","backboard"] as const).map(provider=><div className={styles.provider} key={provider}><strong>{provider==="gemini"?"Gemini / LangChain":"Backboard"}</strong><small>{status?.[provider].configured?checks[provider]?`Verified ${checks[provider]}`:"Key configured · unverified":"Not configured"}</small>{provider==="gemini"&&<small>{status?.gemini.model}</small>}<button disabled={locked} onClick={()=>run(`Checking ${provider}`,async()=>{setChecks(c=>{const next={...c};delete next[provider];return next;});const s=await call<Status>("status");setStatus(s);const r=await call<{verifiedAt:string}>("check",{provider});setChecks(c=>({...c,[provider]:new Date(r.verifiedAt).toLocaleTimeString()}));})}>Check {provider}</button></div>)}
      ...
      <div className={styles.workspace}>
        ...
        <div hidden={mode!=="Chat"}>
          <div className={styles.toolbar}><h3>Gemini conversation</h3><button disabled={locked||!turns.length} onClick={()=>{setTurns([]);setLastRequest(null);setUsed({memories:[],warnings:[]});setError("");setNotice("Started a new conversation. Saved Backboard notes are unchanged.");}}>New conversation</button></div>
          <div className={styles.messages} aria-label="Conversation">
            {!turns.length&&<div className={styles.empty}><h3>Start with something real.</h3><p>Ask about a project, rehearse an explanation, or test how Gemini uses your confirmed experience.</p><button onClick={()=>setMessage("Help me practice explaining a technical tradeoff. Ask me one question at a time.")}>Try a practice prompt</button></div>}
            {turns.map(t=><article key={t.id} className={t.role==="user"?styles.user:styles.assistant}><strong>{t.role==="user"?"You":"Gemini"}{t.role==="assistant"&&!t.complete?" · incomplete":""}</strong><p>{t.content||"Waiting for Gemini…"}</p></article>)}<div ref={bottom}/>
          </div>
          <form onSubmit={e=>{e.preventDefault();send();}}><label className={styles.field}>Message Gemini<textarea value={message} maxLength={16000} rows={3} onChange={e=>setMessage(e.target.value)} placeholder="Ask a question or rehearse an answer…" disabled={streaming}/></label><div className={styles.actions}><button className={styles.primary} disabled={locked||!message.trim()} type="submit">Send message ↗</button>{streaming?<button type="button" onClick={()=>abort.current?.abort()}>Stop response</button>:<button type="button" disabled={!!busy||!lastRequest} onClick={()=>{if(lastRequest)void chat(lastRequest);}}>Retry last message</button>}</div></form>
        </div>
        <div hidden={mode!=="Interview"}>
          ...
          <p>The selected context personalizes this pack. Corpus adaptations retain their original IDs; new questions are labeled.</p>
          <button className={styles.primary} disabled={locked||count<1||count>10} onClick={()=>run("Generating personalized questions",async()=>{const r=await call<{pack:QuestionPack;usedContext:UsedContext}>("questions",{context,count});setPack(r.pack);setQuestionId(r.pack.questions[0].id);setAnswer("");setReport(null);setDraftNotes([]);setUsed(r.usedContext);})}>Generate question pack →</button>
          {pack&&<><p className={styles.caption}>{pack.model} · generated {new Date(pack.createdAt).toLocaleTimeString()}</p><div className={styles.questions}>{pack.questions.map((q,i)=><button key={q.id} aria-pressed={question?.id===q.id} disabled={locked} onClick={()=>{setQuestionId(q.id);setAnswer("");setReport(null);setDraftNotes([]);}}><small>{String(i+1).padStart(2,"0")} / {q.category}</small><span>{q.prompt}</span><small>{q.origin==="corpus-personalized"?`Adapted from ${q.sourceQuestionId}`:"New Gemini question"}</small></button>)}</div></>}
