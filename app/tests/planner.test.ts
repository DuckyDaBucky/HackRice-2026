...
describe("interview planning",()=>{
 it("uses the parsed level and ignores user target overrides",()=>{expect(effectiveLevel({profile,useMemory:false})).toBe("intern");expect(effectiveLevel({profile,target:{...target,level:"senior"},useMemory:false})).toBe("intern");});
